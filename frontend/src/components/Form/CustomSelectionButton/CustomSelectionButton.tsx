...
export const CustomSelectionButton: React.FC<CustomSelectionButtonProps> = ({
    ...
    return (
        <button
            ...
            onClick={onClick}
            disabled={active}
            className={`flex flex-col items-center border-2 transition duration-200 ease-in-out ${active ? `border-${activeColor}-500` : 'border-gray-100'
        ...
        >
            ...
            {icon && <Icon icon={icon} className={`${active ? `text-${activeColor}-500` : 'text-gray-300'
                } text-4xl`} />}
