            ...
            {(props: FieldProps<string | number | boolean>) => (
                <div className={`relative ${hasMargin ? 'mb-2 md:mb-4' : ''}`}>
                    ...
                    <button
                        ...
                        className={`relative my-1 flex w-full items-center justify-between rounded-lg border border-gray-100 bg-gray-100 px-4 py-2 sm:mb-0 ${isOpen ? 'outline outline-2 outline-offset-2 outline-black' : ''
                    ...
                    >
                        ...
                        <Icon
                            icon="expand_more"
                            className={`ml-6 transform-gpu transition-transform duration-200 ease-linear ${isOpen ? '-rotate-180' : 'rotate-0'
                                }`}
                        ></Icon>
                    </button>
                    ...
                    {isOpen && (
                        <div
                            ...
                            className="absolute z-10 w-full rounded-lg bg-white py-2 mt-1 shadow-[-10px_10px_10px_rgba(203,210,217,0.10),10px_10px_10px_rgba(203,210,217,0.10)]"
                            tabIndex={-1}
                        >
                            {options.map((option) => (
                                <button
                                    ...
                                    className={`flex w-full items-center px-5 py-2 hover:text-red ${props.field.value === option.value ? 'text-red' : 'text-gray-700'
                                        }`}
                                >
