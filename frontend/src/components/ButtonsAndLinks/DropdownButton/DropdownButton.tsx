...
export const DropdownButton: React.FC<DropdownButtonProps> = ({ datacy, children, options, buttonProps }) => {
    ...
    return <div className="relative mb-2 md:mb-4">
        ...
        {/* Input */}
        <Button datacy={datacy} className="relative" {...buttonProps} onClick={() => setIsOpen(true)}>{children}</Button>
        ...
        {isOpen && (
            <div className="absolute z-10 mt-1 w-max rounded-xl bg-white py-2 shadow-[-10px_10px_10px_rgba(203,210,217,0.10),10px_10px_10px_rgba(203,210,217,0.10)]" tabIndex={-1}>
                {options.map((option, index) => (
                    <button datacy={`dropdownbutton-option-${index}`} type="button" className="flex w-full items-center text-gray-700 hover:text-black px-5 py-2" key={option.label} onClick={() => {
                        setIsOpen(false)
                        option.onClick()
                    }}>
